...
function pluginHost() {
  return window.WRITE_THEN_PUBLISH_HOST || null;
}

function isPluginMode() {
  return Boolean(pluginHost()?.isPlugin);
}

function appShell() {
  return pluginHost()?.root || document.body;
}

function appRootEl() {
  return pluginHost()?.root || document.documentElement;
}

function appQuery(selector) {
  const root = pluginHost()?.root;
...
const $ = (selector) => appQuery(selector);
...
function initElements() {
  ...
  historyFilterButtons: appQueryAll("[data-history-filter]"),
  panelResizers: appQueryAll("[data-panel-resize]"),
  modeButtons: appQueryAll(".mode-switch [data-app-mode]"),
  ...
  keepHeading: $("#keepHeadingBtn"),
  articleThemeButtons: appQueryAll("[data-article-theme]"),
  articleFontButtons: appQueryAll("[data-article-font]"),
  articleSizeButtons: appQueryAll("[data-article-size]"),
  articleColorButtons: appQueryAll("[data-article-color]"),
  ...
  pairRatioButtons: appQueryAll("[data-pair-ratio]"),
  cropModal: $("#cropModal"),
  ...
  ratioButtons: appQueryAll("[data-ratio]"),
  ...
  livePhotoDurationButtons: appQueryAll("[data-live-duration]"),
  livePhotoSpeedButtons: appQueryAll("[data-live-speed]"),
  livePhotoSpeedHint: $("#livePhotoSpeedHint"),
  livePhotoRatioButtons: appQueryAll("[data-live-ratio]"),
  ...
  featureBadges: appQueryAll("[data-feature-badge]"),
  };
}
...
function defaultFormState() {
  return {
    ...
    textColor: CARD_THEME_COLORS.light.textColor,
    accentColor: CARD_THEME_COLORS.light.accentColor,
    bgColor: CARD_THEME_COLORS.light.bgColor,
...
修改完成后点击“同步回 Obsidian”。浏览器允许写入时会保存到仓库的“成稿预览”文件夹；无法直写时会下载 ZIP 导入包。
...
function syncGuideReadOnlyMode() {
  ...
  appShell().dataset.guideReadonly = readOnly ? "true" : "false";
  ...
  if (readOnly) {
    appQueryAll(".editor-controls details[open]").forEach((details) => details.removeAttribute("open"));
...
function readForm() {
  return {
    ...
    imageHeight: clamp(Number(els.imageHeight.value) || CARD_MAX_IMAGE_HEIGHT, 120, CARD_MAX_IMAGE_HEIGHT),
    headerMode: state.headerMode === "first" ? "first" : "every",
    keepHeadingWithBody: state.keepHeadingWithBody !== false,
...
function normalizeAuthorProfile(data = {}) {
  const displayName = String(data.displayName || "").trim().slice(0, 40) || "作者";
...
function applyForm(data) {
  ...
  updateAvatarPreview();
  appRootEl().style.setProperty("--brush-color", els.inlineColor.value);
  appRootEl().style.setProperty("--text-bg-brush-color", els.inlineBgColor.value);
  updateImageList();
}
...
function setUiTheme(theme, announce = false, syncCard = false) {
  ...
  appRootEl().dataset.uiTheme = nextTheme;
...
function updateKeepHeadingButton() {
  if (!els.keepHeading) return;
...
function updateAppMode() {
  appShell().dataset.appMode = state.appMode;
  const modeButtons = appQueryAll("#appModeSwitch [data-app-mode], .mode-switch [data-app-mode]");
  ...
  if (els.convertMode) {
    ...
    const label = els.convertMode.querySelector("span");
    if (label) label.textContent = targetLabel;
  }
  if (els.articleSettings) els.articleSettings.hidden = state.appMode !== "article";
  if (els.cardSettings) els.cardSettings.hidden = state.appMode !== "cards";
  if (els.downloadZip) els.downloadZip.hidden = state.appMode === "article";
  if (els.downloadArticle) els.downloadArticle.hidden = state.appMode !== "article";
  if (els.copyWechat) els.copyWechat.hidden = state.appMode !== "article";
...
async function setAppMode(mode) {
  ...
  const already = state.appMode === nextMode;
  state.appMode = nextMode;
  updateAppMode();
  if (nextMode === "cards") showCardRenderPlaceholder();
  if (!already || nextMode === "cards") await render();
}
...
function startPanelResize(event) {
  ...
  appShell().classList.add("resizing-panels");
  ...
  const stop = (upEvent) => {
    ...
    appShell().classList.remove("resizing-panels");
...
async function submitFeedback(event) {
  ...
  payload.append("_subject", `【成稿预览】问题反馈 ${feedbackId}`);
...
function showEntryChoice(message = "", tone = "") {
  ...
  appShell().classList.add("entry-choice-pending");
...
async function initializeCloudAccount() {
  if (LOCAL_DEPLOYMENT_MODE) {
    appShell().classList.add("local-deployment");
    appShell().classList.remove("cloud-session-checking");
  ...
  appShell().classList.toggle("cloud-session-checking", Boolean(api?.configured));
  updateAccountUi();
  cloudState.initialized = true;
...
async function signOutAccount() {
  ...
  try {
    ...
    appShell().classList.remove("entry-choice-pending");
...
function migrateStoredState(data) {
  ...
  data.keepHeadingWithBody = data.keepHeadingWithBody !== false;
...
function showOnboardingStep(index) {
  ...
  closeAccountMenu();
  appShell().classList.toggle("onboarding-account-step", onboardingMode === "whats-new" && step.target === "#accountBtn");
...
function finishOnboarding({ remember = true } = {}) {
  ...
  appShell().classList.remove("onboarding-account-step");
  closeAccountMenu();
  if (els.onboardingAction) {
...
function setCurrentTextColor(color) {
  ...
  appRootEl().style.setProperty("--brush-color", normalized);
...
function setCurrentBgColor(color) {
  ...
  appRootEl().style.setProperty("--text-bg-brush-color", normalized);
...
function textareaHasActionableSelection() {
  ...
  return Boolean(
    ...
    && appShell().getAttribute("data-guide-readonly") !== "true"
  );
}
...
function hasConnectedObsidianVault() {
  return isPluginMode() || Boolean(obsidianVault.handle || obsidianVault.fileLookup);
}
...
function currentObsidianNotePath() {
  ...
  return `成稿预览/${safeObsidianFileName(title)}.md`;
}
...
async function buildObsidianExport() {
  ...
  for (const imageId of new Set(imageIds)) {
    ...
    const folder = isPluginMode() ? (pluginHost()?.imageFolderName || "图片") : "成稿预览/附件";
...
async function importMarkdownFromPluginHost(markdown, cursor) {
  const host = pluginHost();
  if (!host?.resolveImages) return false;
  const references = extractMarkdownImageReferences(markdown);
  const resolved = await host.resolveImages(references);
  for (const image of resolved) {
    if (Object.values(state.images).some((item) => item.vaultPath === image.vaultPath)) continue;
    state.images[image.id] = {
      src: image.src,
      name: image.name,
      sourcePath: image.vaultPath,
  ...
  els.status.textContent = converted.unresolved.length
    ? `已插入笔记，但有 ${converted.unresolved.length} 张图片没找到`
    : `已从当前仓库读取图片并插入`;
  return true;
}

async function importMarkdownFromConnectedVault(markdown, cursor) {
  if (isPluginMode()) return importMarkdownFromPluginHost(markdown, cursor);
...
function nextContentBlock(blocks, index) {
  for (let next = index + 1; next < blocks.length; next += 1) {
    if (blocks[next].type !== "spacer") return blocks[next];
  }
  return null;
}

function headingFollowKeepHeight(ctx, next, settings, contentWidth, emptyCapacity, headingHeight) {
  if (!next) return 0;
  const bodyLine = Math.ceil(settings.fontSize * settings.lineHeight);
  let follow = bodyLine * 2;
  if (next.type === "table" || next.type === "image" || next.type === "imagePair") {
    follow = Math.max(bodyLine * 2, 72);
  } else {
  ...
  const maxFollow = Math.max(0, emptyCapacity - headingHeight - 8);
  return Math.min(follow, maxFollow);
}
...
async function buildPages(settings) {
  ...
  for (let blockIndex = 0; blockIndex < blocks.length; blockIndex += 1) {
    ...
    if (settings.keepHeadingWithBody !== false && isHeadingBlock(block.type) && lines.length) {
      const topMargin = hasContent && previousBlockType !== "spacer" ? style.marginTop : 0;
      const headingHeight = topMargin + lines.length * lineHeight + style.marginBottom;
      const emptyCapacity = page.bounds.bottom - page.bounds.top;
      const followKeep = headingFollowKeepHeight(
        ctx,
        nextContentBlock(blocks, blockIndex),
        settings,
        contentWidth,
        emptyCapacity,
        headingHeight,
      );
      if (hasContent && y + headingHeight + followKeep > page.bounds.bottom) {
        finishPage();
      }
    }
  ...
  return tokens
    .map((token) => {
      ...
      if (token.underline) {
        ...
        styles.push("text-underline-offset: 4px");
      }
      if (token.bgColor) {
...
const WECHAT_INLINE_TAGS = new Set(["SPAN", "STRONG", "B", "EM", "I", "A", "CODE", "U", "MARK"]);
const WECHAT_MIN_LINE_RATIO = 1.8;

function parseWechatPx(value, fontSizePx = 17) {
  const raw = String(value || "").trim();
  if (!raw || raw === "normal" || raw === "inherit" || raw === "initial") return 0;
  ...
  // 公众号会把无单位行高当成 px。1.35 < 17 就会报“行高小于字号”。
  if (!/[a-z]/i.test(raw) && number <= 4) return number * fontSizePx;
  return number;
}

function wechatSafeColor(value, fallback = "") {
  const raw = String(value || "").trim();
  if (!raw || raw === "transparent" || /^rgba?\(\s*0\s*,\s*0\s*,\s*0\s*,\s*0(?:\.0+)?\s*\)$/i.test(raw)) {
    return fallback;
  }
  if (/var\(|color-mix\(/i.test(raw)) return fallback;
  if (/\bcolor\(/i.test(raw)) {
    const canvas = document.createElement("canvas");
    canvas.width = 1;
    canvas.height = 1;
    const context = canvas.getContext("2d", { willReadFrequently: true });
    const resolved = resolveExportColor(raw, context);
    return /rgba?\(|#[0-9a-f]{3,8}/i.test(resolved) ? resolved : fallback;
  }
  return raw;
}
...
function wechatLineHeightPx(fontSizePx, computedLineHeight) {
  ...
  const minimum = Math.ceil(font * WECHAT_MIN_LINE_RATIO);
  const parsed = parseWechatPx(computedLineHeight, font);
  return Math.max(Math.round(parsed), minimum);
}

function wechatFontFamily(value) {
  const raw = String(value || "").trim();
  if (!raw || /ui-sans-serif|system-ui|-apple-system/i.test(raw)) {
    return 'PingFang SC, Hiragino Sans GB, Microsoft YaHei, sans-serif';
  }
  return raw.replace(/"/g, "").replace(/ui-sans-serif,\s*|system-ui,\s*|-apple-system,\s*|BlinkMacSystemFont,\s*/g, "");
}

function copyComputedWechatStyles(source, target) {
  const computed = window.getComputedStyle(source);
  const fontSize = Math.max(12, Math.round(parseWechatPx(computed.fontSize) || 17));
  const tag = source.tagName;
  const inline = WECHAT_INLINE_TAGS.has(tag);
  const accent = wechatSafeColor(readForm().articleColor, "#0f766e");
  const styles = [];

  const color = wechatSafeColor(computed.color, "#333333");
  if (color) styles.push(`color:${color}`);
  const background = wechatSafeColor(computed.backgroundColor);
  if (background && !["SECTION", "ARTICLE", "FIGURE", "UL", "OL"].includes(tag)) {
    styles.push(`background-color:${background}`);
  }

  styles.push(`font-size:${fontSize}px`);
  styles.push(`font-weight:${computed.fontWeight || 400}`);
  if (computed.fontStyle && computed.fontStyle !== "normal") styles.push(`font-style:${computed.fontStyle}`);
  if (!inline) {
    styles.push(`line-height:${wechatLineHeightPx(fontSize, computed.lineHeight)}px`);
    styles.push("margin:0 0 16px");
  ...
  if (computed.textAlign && computed.textAlign !== "start" && computed.textAlign !== "left" && !inline) {
    styles.push(`text-align:${computed.textAlign}`);
  }
  if (computed.textDecorationLine && computed.textDecorationLine !== "none") {
    styles.push(`text-decoration:${computed.textDecorationLine} ${computed.textDecorationStyle || "solid"}`);
    styles.push("text-underline-offset:4px");
  }

  if (tag === "H1") {
    styles.push("text-align:center", "padding-bottom:12px", `border-bottom:2px solid ${accent}`);
  }
  if (tag === "H2") {
    styles.push("display:inline-block", "padding:4px 10px");
  }
  if (tag === "H3") {
    styles.push("padding:0");
  }
  if (tag === "BLOCKQUOTE") {
    styles.push(`border-left:4px solid ${accent}`, "padding:12px 16px", "margin:0 0 16px");
  }
  if (tag === "UL" || tag === "OL") {
    styles.push("padding-left:1.4em", "margin:0 0 16px");
  }
  if (tag === "LI") {
    styles.push("margin:0 0 8px", "padding:0");
  }
  if (tag === "PRE") {
    styles.push("padding:12px 14px", "overflow:auto", "white-space:pre-wrap");
  }
  if (tag === "CODE" && source.parentElement?.tagName !== "PRE") {
    styles.push("padding:1px 4px");
  }
  if (tag === "TABLE") {
    styles.push("border-collapse:collapse", "width:100%");
  }
  if (tag === "TH" || tag === "TD") {
    styles.push("padding:8px 10px", "border:1px solid #d8dee6", "margin:0");
  }
  if (tag === "IMG") {
    styles.push("display:block", "width:100%", "max-width:100%", "height:auto", "margin:8px auto", "border:0");
    target.removeAttribute("width");
    target.removeAttribute("height");
  }
  if (tag === "FIGURE") {
    styles.push("margin:0 0 16px", "padding:0");
  }
...
function serializeArticleForWechat() {
  ...
  sourceNodes.forEach((source, index) => {
    if (cloneNodes[index]) copyComputedWechatStyles(source, cloneNodes[index]);
  });
  sourceNodes.forEach((source, index) => {
    if (source.matches?.("video, .article-live-badge, .article-live-actions")) cloneNodes[index]?.remove();
  });
  clone.querySelectorAll("br + br").forEach((node) => node.remove());

  const section = document.createElement("section");
  const articleStyle = window.getComputedStyle(article);
  const fontSize = Math.max(15, Math.round(parseWechatPx(articleStyle.fontSize) || 17));
  section.setAttribute(
    "style",
    [
      "margin:0 auto",
      "padding:0",
      `color:${wechatSafeColor(articleStyle.color, "#333333")}`,
      "background-color:#ffffff",
      `font-family:${wechatFontFamily(articleStyle.fontFamily)}`,
      `font-size:${fontSize}px`,
      `line-height:${wechatLineHeightPx(fontSize, articleStyle.lineHeight)}px`,
  ...
  try {
    const response = await fetch("/api/wechat/drafts", {
      ...
      body: JSON.stringify({
        ...
        slug: state.currentProjectId || "chenggao",
...
function selectPreviewImage(selection) {
  ...
  appQueryAll(".preview-image-box").forEach((box) => {
...
function clearPreviewImageSelection() {
  previewImageSelection = null;
  appQueryAll(".preview-image-box.is-selected").forEach((box) => {
...
function stopPreviewImageDrag() {
  ...
  appQueryAll(".page-frame.is-image-drop-target").forEach(clearPreviewImageDropFrame);
}
...
function previewFrameAtPoint(clientX, clientY) {
  ...
  for (const frame of appQueryAll(".page-frame")) {
...
function syncExportBusyState() {
  ...
  appShell().classList.toggle("export-busy", mainBusy || handoffBusy);
  ...
  appQueryAll("[data-page-export]").forEach((button) => {
...
async function generateLivePackageForCanvas(canvas, pageIndex, reveal = true, serviceChecked = false, onStage = null) {
  ...
  if (canRenderInBrowser) {
    const settings = normalizeLiveMediaSettings(image.liveSettings);
    const scale = LIVE_PAGE_WIDTH / CANVAS_WIDTH;
    const wellX = clamp(Math.round(hit.x * scale), 0, LIVE_PAGE_WIDTH);
    const wellY = clamp(Math.round(hit.y * scale), 0, LIVE_PAGE_HEIGHT);
    const result = await browserEngine.renderLivePhoto({
      ...
      well: {
        x: wellX,
        y: wellY,
        width: clamp(Math.round(hit.width * scale), 40, LIVE_PAGE_WIDTH - wellX),
        height: clamp(Math.round(hit.height * scale), 40, LIVE_PAGE_HEIGHT - wellY),
      },
  ...
  const wellX = clamp(Math.round(hit.x * scale), 0, LIVE_PAGE_WIDTH);
  const wellY = clamp(Math.round(hit.y * scale), 0, LIVE_PAGE_HEIGHT);
  const wellWidth = clamp(Math.round(hit.width * scale), 40, LIVE_PAGE_WIDTH - wellX);
  const wellHeight = clamp(Math.round(hit.height * scale), 40, LIVE_PAGE_HEIGHT - wellY);
...
async function prepareCloudLivePhotoBatch() {
  ...
  return {
    ...
    archive_name: `${safeObsidianFileName(projectTitleFromData(readForm())) || "成稿预览"}-全部内容.zip`,
...
async function chooseSaveTarget(filename, mimeType, extension) {
  if (isPluginMode()) return { plugin: true, filename };
...
async function downloadAll() {
  ...
  if (isPluginMode()) {
    await downloadCanvasesToPluginVault();
    return;
  }
...
function positionOpenToolPopovers() {
  appQueryAll(".tool-menu[open]").forEach(positionToolPopover);
}

function bindEvents() {
  appQueryAll(".tool-menu").forEach((menu) => {
    const summary = menu.querySelector("summary");
    const popover = menu.querySelector(".tool-popover");
    summary?.addEventListener("click", () => {
      if (menu.open) return;
      appQueryAll(".tool-menu").forEach((other) => {
  ...
  document.addEventListener("pointerdown", (event) => {
    ...
    appQueryAll(".tool-menu[open]").forEach((menu) => {
      menu.open = false;
    });
    hideSelectionToolbar();
  });
  ...
  appQueryAll(".toolbar [data-format]").forEach((button) => {
    button.addEventListener("click", () => {
  ...
  const modeSwitch = $("#appModeSwitch") || appQuery(".mode-switch");
  modeSwitch?.addEventListener("click", (event) => {
    const button = event.target.closest("[data-app-mode]");
    if (!button || !modeSwitch.contains(button)) return;
    event.preventDefault();
    void setAppMode(button.dataset.appMode);
  });
  els.modeButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      event.preventDefault();
      void setAppMode(event.currentTarget.dataset.appMode);
    });
  });

  els.articleThemeButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      void setArticleOption("theme", event.currentTarget.dataset.articleTheme);
      event.currentTarget.closest("details")?.removeAttribute("open");
    });
  });
  els.articleFontButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      void setArticleOption("font", event.currentTarget.dataset.articleFont);
      event.currentTarget.closest("details")?.removeAttribute("open");
    });
  });
  els.articleSizeButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      void setArticleOption("size", event.currentTarget.dataset.articleSize);
      event.currentTarget.closest("details")?.removeAttribute("open");
    });
  });
  els.articleColorButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      void setArticleOption("color", event.currentTarget.dataset.articleColor);
      event.currentTarget.closest("details")?.removeAttribute("open");
    });
  });
  ...
  appQueryAll("[data-custom-color-toggle]").forEach((summary) => {
    summary.addEventListener("mousedown", keepTextareaSelection);
  });
  appQueryAll(".selection-custom-color-disclosure").forEach((details) => {
    details.addEventListener("toggle", () => {
      if (!details.open || els.selectionToolbar?.hidden) return;
      window.requestAnimationFrame(showSelectionToolbar);
    });
  });
  appQueryAll(".selection-toolbar [data-format]").forEach((button) => {
    button.addEventListener("mousedown", keepTextareaSelection);
    button.addEventListener("click", () => {
      wrapSelection(button.dataset.format);
  ...
  appQueryAll("[data-custom-color-action]").forEach((button) => {
    button.addEventListener("mousedown", keepTextareaSelection);
    button.addEventListener("click", () => {
      applyCustomColorChoice(button.dataset.colorKind, button.dataset.colorSave === "true");
    });
  });
  appQueryAll("[data-clear-inline-style]").forEach((button) => {
    button.addEventListener("mousedown", keepTextareaSelection);
    button.addEventListener("click", () => {
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if ((event.metaKey || event.ctrlKey) && !event.altKey && event.key.toLowerCase() === "f") {
      const target = event.target;
      const insideEditor = target === appShell() || target.closest?.(".editor-panel, .tool-menu");
      if (insideEditor) {
        event.preventDefault();
...
function schedulePluginNoteWrite() {
  if (!isPluginMode() || pluginNoteHydrating || pluginHost()?.previewOnly || !pluginHost()?.writeNote) return;
  window.clearTimeout(pluginNoteWriteTimer);
  pluginNoteWriteTimer = window.setTimeout(() => {
    void persistPluginNote();
  }, 700);
}

async function persistPluginNote() {
  const host = pluginHost();
  if (!host?.writeNote || host.previewOnly || pluginNoteHydrating) return;
  try {
    const exportData = await buildObsidianExport();
    await host.writeNote({
...
async function loadFormFromPluginNote() {
  const host = pluginHost();
  if (!host?.loadNote) return blankFormState();
  const note = await host.loadNote();
  state.obsidianNotePath = note.path;
  state.currentProjectId = `obsidian_${note.path}`;
  state.images = mergePluginImages(note.images);
  const converted = convertObsidianImageReferences(note.content, state.images);
  const form = blankFormState();
  form.content = converted.content;
  form.images = state.images;
  form.appMode = state.appMode === "article" || state.appMode === "cards"
    ? state.appMode
    : host.preferredMode || "cards";
  form.obsidianNotePath = note.path;
  if (converted.unresolved.length) {
    form._pluginStatus = `已打开笔记，但有图片没找到：${converted.unresolved.slice(0, 3).join("、")}`;
  }
  return form;
}

async function bootWriteThenPublish() {
  LOCAL_DEPLOYMENT_MODE = document.documentElement.dataset.writeThenPublishLocalMode === "true" || isPluginMode();
  initElements();
  if (isPluginMode()) {
    applyPluginHostUi();
    loadPanelLayout();
    applyPanelLayout();
    pluginNoteHydrating = true;
    try {
      const noteForm = await loadFormFromPluginNote();
      applyForm(noteForm);
      if (noteForm._pluginStatus) els.status.textContent = noteForm._pluginStatus;
      else applyPluginHostUi();
    } finally {
      pluginNoteHydrating = false;
    }
    syncGuideReadOnlyMode();
    resetTextHistory();
    updateProjectHistory();
  ...
  if (cloudApi()?.configured) appShell().classList.add("cloud-session-checking");
  loadPanelLayout();
  applyPanelLayout();
...
async function reloadWriteThenPublishNote() {
  ...
  try {
    ...
    state.images = mergePluginImages(note.images);
    const converted = convertObsidianImageReferences(note.content, state.images);
    if (els.content) els.content.value = converted.content;
    state.images = normalizeImagesForContent(converted.content, state.images);
    if (converted.unresolved.length) {
      els.status.textContent = `已打开笔记，但有图片没找到：${converted.unresolved.slice(0, 3).join("、")}`;
    }
    await render();
    if (window.lucide) window.lucide.createIcons();
  } finally {
